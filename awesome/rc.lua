...
require("debian.menu")
--start compositing (required here, mysteriously).
...
--keymap
-- os.execute('setxkbmap -option compose:ralt') use this on machines that don't need a special keymap
--disable below on machines without a .Xmodmap needed
os.execute('xmodmap .Xmodmap')

scratch.drop("urxvt -e byobu","top","center",1, 0.5, true)
--start network app
...
beautiful.init("/home/dmorris/.config/awesome/themes/psyburn/theme.lua")
...
--menubar added 11.24.2013
require('menubar')
menubar.cache_entries = true
menubar.app_folders = { "/usr/share/applications/" }
menubar.show_categories = true
menubar.set_icon_theme("Tango")

--redshift copypasta (no worky)
--local redshift = require("redshift")
--redshift.redshift = "/usr/bin/redshift"
--redshift.options = "-l 38:77"
--redshift.init(1)



-- DELIGHTFUL WIDGETS, NEW, DELETE IF MISBEHAVIOR OCCURS
-- Delightful widgets
...
-- removed imap, may have fucked parens.

    [delightful.widgets.memory] = {
        command = 'gnome-system-monitor',
    },
-- removed weather, may have fucked parens.
...
--    awful.key({ modkey,           }, "d", redshift.toggle               ),
    ...
    awful.key({ "Mod1", "Control" }, "l", function() os.execute("xscreensaver-command -lock &") end),
    awful.key({ modkey,           }, "j",
        function ()
            awful.client.focus.byidx( 1)
            if client.focus then client.focus:raise() end
...
awful.util.spawn(terminal) end), awful.key({ modkey, "Control" }, "r", 
awesome.restart),    awful.key({ modkey, "Shift"   }, "q", awesome.quit),
    ...
    awful.key({ modkey,           }, "`", function () scratch.drop("urxvt -e byobu","top","center",1,0.5,true) end),
    -- Prompt
    awful.key({ modkey },            "r",     function () mypromptbox[mouse.screen]:run() end),
    --hack for run in term⎈
    awful.key({ modkey }, "t",
              function ()
                  awful.prompt.run({ prompt = " In Term: " },
